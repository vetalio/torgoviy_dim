...
// Функція для витягування Calendar ID з URL
...
function copyCardNumber() {
    if (typeof CARD_NUMBER !== 'undefined' && CARD_NUMBER) {
        copyToClipboard(CARD_NUMBER.replace(/\s/g, ''), 'copyCardNumberButton', '✓ Номер картки скопійовано', false);
    }
}
function copyCardHolder() {
    if (typeof CARD_HOLDER_NAME !== 'undefined' && CARD_HOLDER_NAME) {
        copyToClipboard(CARD_HOLDER_NAME, 'copyCardHolderButton', '✓ Прізвище скопійовано', false);
    }
}
function copyCardBank() {
    if (typeof CARD_BANK_NAME !== 'undefined' && CARD_BANK_NAME) {
        copyToClipboard(CARD_BANK_NAME, 'copyCardBankButton', '✓ Назва банку скопійовано', false);
    }
}
            ...
            if (calendarIframe) {
                ...
                setTimeout(() => {
                    try {
                        ...
                        // CORS помилка - це нормально, це це нормально
